...
if ($_SESSION["SchoolType"] == 1 && $_SESSION["SchoolID"] != 8 && $_SESSION["SchoolID"] != 18) {
	...
	switch ($level_klas) {
		case "1":
			$spreadsheet = $reader->load("../templates/verza_ps_1.xlsx");
	...
	while ($i <= $rap_in) {
		...
		while ($row = mysqli_fetch_assoc($resultado)) {
			...
			if ($row["gemiddelde"] > 0.0 && ($level_klas != 1 || $i != 1)) {
		...
		case 2:
			...
			while ($i <= $_GET["rapport"]) {
								...
								while ($select->fetch()) {									/* initialize last student id if whilecounter == 0 */
									...
									switch ($x_index_out) {
										...
										case 26:
											...
											// case 63:
											// 	$returnvalue = ($level_klas == 2 ? "Q" : "");
											// 	break;

											// case 64:
											// 	$returnvalue = ($level_klas == 2 ? "R" : "");
											// 	break;
									...
									$colgemiddelde = (string)$returnvalue . (string)$_current_student_start_row;
									if ($gemiddelde_out > 0.0 && $gemiddelde_out != null &&  $returnvalue != "XX") {
				...
				ORDER BY v.created";
					...
					$hojaActiva->setCellValue("AJ" . (string)$_current_student_start_row, $cont_huis);
		...
		case 3:
			...
			while ($i <= $_GET["rapport"]) {
								...
								while ($select->fetch()) {
									...
									while ($cont_houdin < 19) {
										switch ($cont_houdin) {
											...
											case 4:
												$_h4 = "";
												$colhouding = "W" . (string)$_current_student_start_row;
												if ($h4 == 1 || !isset($h4)) {
													$_h4 = "B";
												}
												if ($h4 == 2) {
											...
											case 15:
												$_h15 = "";
												$colhouding = "I" . (string)$_current_student_start_row;
												if ($h15 == 1 || !isset($h15)) {
													$_h15 = "B";
												}
												if ($h15 == 2) {
				...
				ORDER BY v.created";
					...
					$hojaActiva->setCellValue("AQ" . (string)$_current_student_start_row, $cont_laat);
					$hojaActiva->setCellValue("AR" . (string)$_current_student_start_row, $cont_verzuim);
					$hojaActiva->setCellValue("AI" . (string)$_current_student_start_row, $cont_huis);
					$hojaActiva->setCellValue("AS" . (string)$_current_student_start_row, $opmerking);
					$cont_laat = 0;
					$cont_verzuim = 0;
					$cont_huis = 0;
					$opmerking = "";
		...
		case 6:
						...
						if ($select->execute()) {
							...
							if ($select->num_rows > 0) {
								...
								while ($select->fetch()) {									/* initialize last student id if whilecounter == 0 */
									...
									$colgemiddelde = (string)$returnvalue . (string)$_current_student_start_row;
									if ($gemiddelde_out > 0.0 && $returnvalue !== "ZZ") {
										$hojaActiva->setCellValue($colgemiddelde, $gemiddelde_out);
									}
									$_laststudent = $_currentstudent;
									$_while_counter++;
			...
			ORDER BY s.sex ASC,s.lastname ASC, s.firstname";
				...
				ORDER BY v.created";
					...
					$wer_pro = 0;
					$wer_cont = 0;
					$hojaActiva->setCellValue(($level_klas == 6 ? "BF" : "AX") . (string)$_current_student_start_row, $opmerking);
					$hojaActiva->setCellValue(($level_klas == 6 ? "BD" : "AV") . (string)$_current_student_start_row, $cont_laat);
					$hojaActiva->setCellValue(($level_klas == 6 ? "BE" : "AW") . (string)$_current_student_start_row, $cont_verzuim);
					$hojaActiva->setCellValue(($level_klas == 6 ? "AM" : "AM") . (string)$_current_student_start_row, $cont_huis);
					$sql_query_k5 = "SELECT
						c.gemiddelde,
						v.volledigenaamvak
						FROM le_cijfers c
						...
						AND c.gemiddelde IS NOT NULL  
						AND v.volledigenaamvak IN ('Woordenschat','Luisteren en spreken','Leesbegrip','Dictee','Taalbeschouwing','Reflexion','Vocabulario','Dictado','Scucha y papia','Tabel y grafico','Operacion basico y avansa','Nocion di number','Midi y Geometria','Wereldorientatie','Lesamento Comprensivo')";
					$resultado_k5 = mysqli_query($mysqli, $sql_query_k5);
					if ($i == 1 && $level_klas == 6) {
						$spreadsheet->setActiveSheetIndex(3);
						$hojaActiva = $spreadsheet->getActiveSheet();
						...
						if ($hul_cont > 0)
							$hojaActiva->setCellValue("AT" . (string)$_current_student_start_row, round($hul_pro / $hul_cont, 1));
						if ($mat_cont > 0)
							$hojaActiva->setCellValue("AU" . (string)$_current_student_start_row, round($mat_pro / $mat_cont, 1));
						if ($pap_cont > 0)
							$hojaActiva->setCellValue("AV" . (string)$_current_student_start_row, round($pap_pro / $pap_cont, 1));
						if ($wer_cont > 0)
							$hojaActiva->setCellValue("AW" . (string)$_current_student_start_row, round($wer_pro / $wer_cont, 1));
					}
