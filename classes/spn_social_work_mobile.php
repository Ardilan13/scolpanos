<?php
	...
	class spn_social_work_mobile
	{
		...
		function get_social_work($schooljaar, $id_social_work, $id_student, $dummy)
		{
			...
			if ($dummy)
			{
				$htmlcontrol .= "<table id=\"dataRequest-social_work" ."\" class=\"table table-striped\" data-table=\"yes\">";
				...
				for ($i = 1; $i <= 10; $i++)
				{
					...
					$htmlcontrol .= "<td>". htmlentities($i)."A</td><td>". ($i % 2 == 0? "Pending": "No Pending" ) ."</td></tr>";
				}
			...
			else
			{
				...
				try
				{
					...
					if($select = $mysqli->prepare("CALL ". $this->sp_read_social_work ." (?,?, ?)"))
					{
						if($select->bind_param("sii",$schooljaar, ($id_social_work == "" || $id_social_work == 0 || $id_social_work == "null" ? null : $id_social_work) , $id_student))
						{
							if($select->execute())
							{
								$this->error = false;
