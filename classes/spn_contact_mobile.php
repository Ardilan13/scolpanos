...
class spn_contact_mobile{
  ...
  public $sp_read_contact_parent = "sp_read_contact_parent";
  public $debug = true;
  public $error = "";
  public $errormessage = "";
          ...
          if($stmt->bind_param("isissssssisssi",$tutor, $type, $id_number, $full_name, $address, $email, $mobile_phone, $home_phone, $work_phone, $work_phone_ext, $company, $position_company, $observations, $id_family))
          {
            if($stmt->execute())
            {
              ...
              */
              ...
              $spn_audit->create_audit($_SESSION['UserGUID'], 'Contact','Create Contact',false);
              $result = 1;
              $stmt->close();
              $mysqli->close();
            }
  ...
  function list_contacts($id_family, $id_contact = null)
  {
    ...
    try
    {
      ...
      if($select=$mysqli->prepare("CALL ".$this->sp_read_contact_parent." (?,?)"))
      {

        if($select->bind_param("si",$id_family,$id_contact))
        {
          if($select->execute())
          {
            $this->error = false;
            ...
            if($select->num_rows > 0)
            {
              ...
              $htmlcontrol .= "<table id=\"dataRequest-contact\" class=\"table table-striped\" data-table=\"yes\">";
              $htmlcontrol .= "<thead><tr><th>Tutor</th><th>Type</th><th>Full Name</th><th>Mobile Phone</th><th>Email</th><th>Address</th><th>Company</th><th>Position</th></tr></thead>";
              $htmlcontrol .= "<tbody>";
              ...
              while($select->fetch())
              {
                ...
                //$htmlcontrol .="<tr><td>". htmlentities($full_name) ."</td><td>". htmlentities($email) ."</td><td>". htmlentities($mobile_phone) ."</td><td>". htmlentities($home_phone) ."</td><td>". htmlentities($type) ."</td><td>s</td></tr>";
                $htmlcontrol .= "<tr><td>". ($tutor==1?"<i name='check_tutor' class='fa fa-check'></i>":"<i></i>") ."</td><td>". htmlentities($type) ."</td><td>". htmlentities(utf8_encode($full_name)) ."</td><td>". htmlentities($mobile_phone) ."</td><td>". htmlentities($email) ."</td><td>". htmlentities($address) ."</td><td>". htmlentities($company) ."</td><td>". htmlentities($position_company);
                ...
                if (htmlentities($email) != '' || htmlentities($email) != null){
                  $have_email = true;
                }
              }
              $htmlcontrol .= "</td></tr>";
              $htmlcontrol .= "</tbody>";
              $htmlcontrol .= "</table>";
              $htmlcontrol .= "<input type='hidden' name='have_email' id='have_email' value='".$have_email. "'>";
  ...
  function update_contact( $tutor,$type,$id_number,$full_name,$address,$email,$mobile_phone,$work_phone,$home_phone,$work_phone_ext,$company,$position_company,$observations,$id_contact)
  {
    $result = 0;
    ...
    try
    {
      ...
      {
        if($stmt->bind_param("iissssssssssss",$id_contact,$tutor,$type,$id_number,$full_name,$address,$email,$mobile_phone,$home_phone,$work_phone,$work_phone_ext,$company,$position_company,$observations))
        {
          if($stmt->execute())
          {   //Audit by Caribe Developers
            require_once ("spn_audit.php");
      ...
      if($stmt=$mysqli->prepare("CALL sp_delete_contact (?)"))
      {
        ...
        ))
        {
          if($stmt->execute())
          {
            ...
            require_once ("spn_audit.php");
            $spn_audit = new spn_audit();
            $spn_audit->create_audit($_SESSION['UserGUID'], 'Contact','Delete Contact',false);
            $result = 1;
            $stmt->close();
            $mysqli->close();
          }
