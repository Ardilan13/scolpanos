...
class spn_rapport_school_13_k_3_4
{
	...
	function _openexceltemplate()
	{
		...
		$this->rapport_modified = $this->rapport_reader->load("../templates/13_abrahamdeveer_rapport_file_k3-4.xlsm");
	...
	function _exportexcelfile($exporttobrowser = true)
	{
		...
		if($exporttobrowser)
		{
			...
			header('Content-Disposition: attachment; filename="13_abrahamdeveer_rapport_file.xlsx"');
			/* add headers for output to browser */
		}
					...
					if($select->num_rows > 0)
					{
						...
						$select->bind_result(
							...
							while($select->fetch())
							{
								...
								$this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue($column_houding, $houding_result);
								...
								if ($rap_in == 1){
									...
									if($vakid_out==8961 || $vakid_out==9428 ||$vakid_out==10653 ){
								...
								if ($rap_in == 3){
									...
									if($vakid_out==8852 || $vakid_out==9809 ||$vakid_out==10499 ){
