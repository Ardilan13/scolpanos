...
class spn_rapport_school_9
{
	...
	function _openexceltemplate()
	{
		...
		$this->rapport_modified = $this->rapport_reader->load("../templates/9_Reina_beatrix.xlsm");
	...
	function _exportexcelfile($exporttobrowser = true)
	{
		...
		if($exporttobrowser)
		{
			...
			header('Content-Disposition: attachment; filename="Reina_beatrix_rapport.xlsm"');
			/* add headers for output to browser */
		}
					...
					if($select->execute())
					{
						...
						if($select->num_rows > 0)
						{
							...
							while($select->fetch())
							{
								...
								if($vakid_out==499603 || $vakid_out==498650 || $vakid_out==498962 || $vakid_out==499325 || $vakid_out==499031 || $vakid_out==499371 || $vakid_out==499901 || $vakid_out==498590 || $vakid_out==498690 || $vakid_out==499276 || $vakid_out==499105 || $vakid_out==499279 || $vakid_out==499206 || $vakid_out==498715 || $vakid_out==498973 || $vakid_out==498721 || $vakid_out==499700 || $vakid_out==499789 || $vakid_out==498900 || $vakid_out==498969 || $vakid_out==499652 || $vakid_out==498861 || $vakid_out==498927 || $vakid_out==499561 || $vakid_out==499517 ){
									$this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue("AG" . (string)$_current_student_start_row, $gemiddelde_out); }
