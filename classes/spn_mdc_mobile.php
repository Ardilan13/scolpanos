...
class spn_mdc_mobile
{
  ...
  function get_mdc($schooljaar,$id_mdc, $id_student, $dummy)
  {
    ...
    if ($dummy)
    {
      $htmlcontrol .= "<table id=\"dataRequest-mdc" ."\" class=\"table table-striped\" data-table=\"yes\">";
    ...
    else
    {
      ...
      try
      {
        ...
        if($select = $mysqli->prepare("CALL ". $this->sp_read_mdc ." (?,?, ?)"))
        {
          if($select->bind_param("sii", $schooljaar, ($id_mdc == "" || $id_mdc == 0 || $id_mdc == "null" ? null : $id_mdc) , $id_student))
          {
            if($select->execute())
            {
              $this->error = false;
