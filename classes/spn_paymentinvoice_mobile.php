...
class spn_paymentinvoice_mobile
{
  ...
  function listpaymentinvoice($baseurl,$detailpage)
  {
    $returnvalue = "";
    $user_permission="";
    $sql_query = "";
    ...
    try
    {
      ...
      if ($select =$mysqli->prepare("CALL " . $this->sp_read_listpaymentinvoice . "(?)"))
      {
        ...
        if ($select->bind_param("i",$id_payment))
         ...
         if($stmt=$mysqli->prepare("CALL " . $this->$sp_get_invoice_by_student . "(?)"))
         {
           if ($stmt->bind_param("i",$studentid))
           {
             ...
             if($stmt->execute())
             {
               ...
               {
                 $htmlcontrol .= "<table id=\"tbl_invoice_by_idstudent\" class=\"table table-striped\" data-table=\"no\">";
                 $htmlcontrol .= "<thead>";
