...
class spn_see_kaart
{
  ...
  function list_cijfers_by_student_se_kaart_rapport_ps($schooljaar, $studentid, $rapnummer, $dummy)
  {
    $table = "";
    $average_divisor = 0;
    $is_ckv = false;
    ...
    if ($dummy)
      ...
      try {
        ...
        if ($schooljaar >= "2021-2022") {
          $select = "SELECT id,vak_naam from le_vakken_ps WHERE type = 'c'";
          $resultado = mysqli_query($mysqli, $select);
          while ($row = mysqli_fetch_assoc($resultado)) {
            $vak = $row['id'];
            $complete_name = $row['vak_naam'];
            if ($stmt = $mysqli->prepare("SELECT MAX(CASE WHEN rapnummer = 1 THEN gemiddelde END) Rap1,
                                                 MAX(CASE WHEN rapnummer = 2 THEN gemiddelde END) Rap2,
                                                 MAX(CASE WHEN rapnummer = 3 THEN gemiddelde END) Rap3 
                                          FROM le_cijfers_ps WHERE schooljaar = ? and  studentid = ? and rapnummer <= ? and vak = $vak")) {
              if ($stmt->bind_param("ssi", $schooljaar, $studentid, $rapnummer)) {
                if ($stmt->execute()) {

                  $result = 1;
                  $stmt->bind_result($gemmindele1, $gemmindele2, $gemmindele3);
                  $stmt->store_result();
                  if ($stmt->num_rows > 0) {
                      ...
                      if (strpos($complete_name, 'CKV')) {
                        ...
                        if (!$is_ckv) {
                          ...
                          for ($x = 0; $x <= $rapnummer; $x++) {
                            ...
                            if ((float)$avg_rap2 >= 0 && $avg_rap2 != '' && $avg_rap2 != null) {
                              $average_ckv_divisor++;
                            }
                            if ((float)$avg_rap3 >= 0 && $avg_rap3 != '' && $avg_rap2 != null) {
                              $average_ckv_divisor++;
                            }
                          ...
                          $table .= "<td " . ((float)$final_avg_CKV >= 1 && (float)$final_avg_CKV <= 4.4  && (float)$final_avg_CKV ? "class=\"bg-danger\"" : "") . ">" . ((float)$final_avg_CKV > 0.0 && $final_avg_CKV != null  && $final_avg_CKV != "" ? round($final_avg_CKV) : "") . "</td>";
                          $is_ckv = true;
                        }
                      } else {
                        $table .= "<td width='65%'>$complete_name</td>";
                        ...
                        $table .= "<td " . ((float)$average >= 1 && (float)$average <= 4.4  && (float)$average ? "class=\"bg-danger\"" : "") . ">" . ((float)$average > 0.0 && $average != null  && $average != "" ? round($average) : "") . "</td>";
  ...
  function list_cijfers_by_student_se_kaart_rapport($schooljaar, $studentid, $rapnummer, $klas, $dummy)
  {
