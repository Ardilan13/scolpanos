...
class spn_houding_hs3
{
  ...
  function _updatehouding($houding_id,$schooljaar,$schoolid,$studentid_in,$houding_number_in,$houding_value_in,$klas_in,$rap_in)
  {
    ...
    try
    {
      ...
      $sql_query = "update" . chr(32) . $this->tablename_houding . chr(32) . "h join students_groups s on h.studentid = s.studentid set" . chr(32) . "  h." . $houding_number_in . chr(32) . " = ? where s.studentid = ? and h.klas = ? and h.rapnummer = ? and h.id = ?;";
              ...
              while($select->fetch())
              {
                ...
                for($y=1;$y <= $index_colum;$y++)
                {
                  ...
                  if (($s->_setting_rapnumber_1 && $rap_in == 1) || ($s->_setting_rapnumber_2 && $rap_in == 2) || ($s->_setting_rapnumber_3 && $rap_in == 3))
                  {
                    ...
                    {
                      $htmlcontrol .= "<option value=\"1\"" . ($_houding_number == 1 ? "selected": "") . ">A</option><option value=\"2\"" . ($_houding_number == 2 ? "selected" : "") . ">B</option><option value=\"3\"" . ($_houding_number == 3 ? "selected" : "") . ">C</option><option value=\"4\"" . ($_houding_number == 4 ? "selected" : "") . ">D</option></select></td> ";
                    }
  ...
  function listhouding_hs($schooljaar, $schoolid,$klas_in,$rap_in, $vak_id)
  {
    ...
    $sql_query = "SELECT s.studentid, h.id, h.klas, st.firstname, st.lastname, st.sex,
    h.h1, h.h2, h.h3, h.h4, h.h5, h.h6, h.h7, h.h8, h.h9, h.h10, h.h11, h.h12, h.h13, h.h14, h.h15,
    h.h16,h.h17,h.h18,h.h19,h.h20,h.h21,h.h22,h.h23,h.h24,h.h25
    FROM students_groups s LEFT JOIN le_houding_hs h ON s.studentid = h.studentid
    inner join students st on s.studentid = st.id
    WHERE s.vakid = $vak_id AND h.rapnummer = $rap_in AND s.schoolid = $schoolid and h.schooljaar = '$schooljaar' and h.vakid= $vak_id and h.klas= '$klas_in'
    ORDER BY ";

    $sql_order = " st.lastname, st.firstname ";
    if ($s->_setting_mj)
    {
      $sql_query .= " st.sex ". $s->_setting_sort . ", " . $sql_order;
    }
    else
    {
      $sql_query .=  $sql_order;
  ...
  function create_le_houding_student_hs($schoolID,$rapnummer,$klass,$schooljaar, $vak_id)
  {
    ...
    try
    {
      $mysqli=new mysqli($DBCreds->DBAddress, $DBCreds->DBUser, $DBCreds->DBPass, $DBCreds->DBSchema, $DBCreds->DBPort);
      $mysqli->set_charset('utf8');
      if($stmt=$mysqli->prepare("CALL sp_create_le_houding_student_hs3 (?,?,?,?,?)"))
      {
