...
class spn_rapport_school_17
{
    ...
    function _openexceltemplate()
    {
        ...
        $this->rapport_modified = $this->rapport_reader->load("../templates/17_monplaisir_rapport_file_1.xlsm");
    ...
    function _exportexcelfile($exporttobrowser = true)
    {
        ...
        if ($exporttobrowser) {
            ...
            header('Content-Disposition: attachment; filename="MonPlaisir_rapport_file_h1.xlsm"');
            /* add headers for output to browser */
        }
 ...
 */                                /* Write Student Name */
                            ...
                            if ($rap_in == 1) {
                                ...
                                if ($vakid_out == 552449 || $vakid_out == 552435) {
                                    $this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue("CR" . (string)$_current_student_start_row, $gemiddelde_out);
                                    $this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue("CS" . (string)$_current_student_start_row, $houding_result);
                                }
                            }
                            if ($rap_in == 2) {
                                $colgemiddelde = PHPExcel_Cell::stringFromColumnIndex(($x_index_out + 1)) . (string)$_current_student_start_row;
                                if ($vakid_out == 552449 || $vakid_out == 552435) {
                                    $this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue("CT" . (string)$_current_student_start_row, $gemiddelde_out);
                                    $this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue("CU" . (string)$_current_student_start_row, $houding_result);
                                }
                                if ($_while_counter > 0) {
                            ...
                            if ($rap_in == 3) {
                                ...
                                if ($vakid_out == 552449 || $vakid_out == 552435) {
                                    $this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue("CV" . (string)$_current_student_start_row, $gemiddelde_out);
                                    $this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue("CW" . (string)$_current_student_start_row, $houding_result);
                                }
                                if ($_while_counter > 0) {
                            ...
                            /* $verzuim_result = $this->_writeeapportdata_verzuim_absent($_SESSION['SchoolID'],$_SESSION['SchoolJaar'],$klas_in,$rap_in,$studentid_out);
								if ($rap_in == 1){	$col_verzuim_telaat = "CV";}
								if ($rap_in == 2){	$col_verzuim_telaat = "CW";}
								if ($rap_in == 3){	$col_verzuim_telaat = "CX";}
								...
								if ($rap_in == 1){	$col_verzuim_absent = "CR";}
								if ($rap_in == 2){	$col_verzuim_absent = "CS";}
								if ($rap_in == 3){	$col_verzuim_absent = "CT";}
                                ...
                                if ($rap_in == 1){	$col_verzuim_uitsturen = "FD";}
                                if ($rap_in == 2){	$col_verzuim_uitsturen = "FE";}
                                if ($rap_in == 3){	$col_verzuim_uitsturen = "FF";}
