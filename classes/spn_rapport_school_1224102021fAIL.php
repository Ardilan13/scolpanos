...
class spn_rapport_school_12
{
	...
	function _openexceltemplate()
	{
		...
		$this->rapport_modified = $this->rapport_reader->load("../templates/12_ceque_rapport_file.xlsm");
	...
	function _exportexcelfile($exporttobrowser = true)
	{
		...
		if($exporttobrowser)
		{
			...
			header('Content-Disposition: attachment; filename="ceque_rapport_file.xlsm"');
			/* add headers for output to browser */
		}
	...
	function _writerapportdata_cijfers($schoolid,$schooljaar,$klas_in,$rap_in)
	{
						...
						$select->bind_result(
							...
							while($select->fetch())
							{
								if($_while_counter == 0)
								{
									...
									$tutor = $this->_writetutorNamForRapport($klas_in,$schoolid,$schooljaar);
									//$tutor = $klas_in . " " . $schoolid . " " . $schooljaar;
								...
								if ($rap_in == 2){
									$x_index_out = $x_index_out +2;
								}
								if ($rap_in == 3){
									$x_index_out = $x_index_out +4;
								}
		...
		function _writerapportdata_houding($klas_in,$vakid_out,$studentid_out,$rap_in)
		{
			...
			try
			{
				...
				$DBCreds = new DBCreds();
				$mysqli=new mysqli($DBCreds->DBAddress, $DBCreds->DBUser, $DBCreds->DBPass, $DBCreds->DBSchema, $DBCreds->DBPort);
				$sql_query = "SELECT h1,h2,h3,h4,h5,h6 FROM le_houding_hs where klas = '$klas_in' and vakid = $vakid_out and studentid = $studentid_out and schooljaar = '$schooljaar' and rapnummer = $rap_in";
		...
		function _writetutorNamForRapport($klas_in,$schoolid,$schooljaar)
		{
			...
			try
			{
				...
				$DBCreds = new DBCreds();
				$mysqli=new mysqli($DBCreds->DBAddress, $DBCreds->DBUser, $DBCreds->DBPass, $DBCreds->DBSchema, $DBCreds->DBPort);
				$sql_query = "select CONCAT(a.FirstName,' ',a.LastName) as tutorname from app_useraccounts a inner join user_hs u on UserGUID = user_GUID where u.klas = '$klas_in' and u.tutor = 'yes' and u.SchoolID = '$schoolid' and u.SchoolJaar = '$schooljaar';";
				//print('Este es el query de houding: '.$sql_query);
