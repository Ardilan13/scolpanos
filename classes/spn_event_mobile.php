...
class spn_event_mobile
{
  ...
  function create_event($event_date,$due_date,$reason,$involved,$observations,$private,$important_notice,$id_student,$dummy)
  {
    $result = 0;
    ...
    try
    {
      ...
      else
      {
        ...
        if($stmt=$mysqli->prepare("CALL " . $this->sp_create_event . " (?,?,?,?,?,?,?,?)"))
        {
          ...
          if($stmt->bind_param("sssssiii", $event_date, $due_date, $reason, $involved,$observations, $private, $important_notice,$id_student))
          {
            if($stmt->execute())
            {
        ...
        if($select->bind_param("sii",$schooljaar, $id_event,$id_student))
        {
          if($select->execute())
          {
            ...
            if($select->num_rows > 0)
            {
              ...
              $htmlcontrol .= "<table id=\"dataRequest-event\" class=\"table table-striped\" data-table=\"yes\">";
