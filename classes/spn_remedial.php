...
class spn_remedial
{
	...
	public $sp_read_remedial_detail_print = "sp_read_remedial_detail_print";
				...
				if ($select->bind_param("si",$schooljaar,$id_student))
				{
					if($select->execute())
					{
						...
						if($select->num_rows > 0)
						{
							...
							$htmlcontrol .= "<table id=\"dataRequest-remedial\" class=\"table table-bordered table-colored\" data-table=\"yes\">";
				...
				if ($select->bind_param("i", $id_remedial))
				{
					if($select->execute())
					{
						...
						if($select->num_rows > 0)
						{
							...
							$htmlcontrol .= "</table>";
							$htmlcontrol .= "<div>";
							$htmlcontrol .= "<a type='submit' name='btn_leerling_inner_detail_remedial_print' id_remedial='$id_remedial' id='btn_leerling_inner_detail_remedial_print'class='btn btn-default btn-m-w pull-right mrg-left' target='_blank'>PRINT</a>";
							$htmlcontrol .= "</div>";
	...
	/// Paramenters in: id remedial detail (optional), id remedial, dummy
	/// Parametres out: html table
	function get_remedial_detail_print($id_remedial_detail, $id_remedial, $dummy)
	{
		$returnvalue = "";
		$sql_query = "";
		$htmlcontrol="";
		...
		try
		{
			...
			if ($select =$mysqli->prepare("CALL " . $this->sp_read_remedial_detail_print ."(?)"))
			{
				if ($select->bind_param("i", $id_remedial))
				{
					if($select->execute())
					{
						...
						$this->error = false;
						$result = 1;
						$select->bind_result($id_remedial_detail, $date, $observation, $id_remedial,$subject,$school_year,$class,$begin_date,$end_date);
						$select->store_result();
						...
						if($select->num_rows > 0)
						{
							...
							while($select->fetch())
							{
								if ($i==0){
									...
									$htmlcontrol .= "<h4>Remedial</h4>";
									$htmlcontrol .= "<table id=\"dataRequest-remedial\" class=\"table table-bordered table-colored\" data-table=\"yes\">";
									$htmlcontrol .= "<thead><tr><th>Subject</th><th>Class</th><th>School Year</th><th>Begin Date</th><th>End Date</th></tr></thead>";
									$htmlcontrol .= "<tbody>";
									...
									$htmlcontrol .= "<h4>Remedial Detail</h4>";
									$htmlcontrol .= "<table id=\"dataRequest-remedial-detail\" class=\"table table-bordered table-colored\" data-table=\"yes\">";
									$htmlcontrol .= "<thead><tr><th>Date</th><th>Observation</th></tr></thead>";
									$htmlcontrol .= "<tbody>";
								}
