...
class spn_message
{
  ...
  public $sp_read_notifications_parents ="sp_read_notifications_parents";
  ...
  public $sp_read_message_parent ="sp_read_message_parent";
  public $get_count_unread_messages_parent  ="get_count_unread_messages_parent";
          ...
          if($select->execute())
          {
            ...
            if($select->num_rows > 0)
            {
              ...
              while($select->fetch())
              {
                ...
                $htmlcontrol .= "</td>";
                $htmlcontrol .= "<td>";
                $htmlcontrol .="<a class='btn btn-white' onclick='deleteMessage(".htmlentities($id).")' href='#'><i class='fa fa-trash-o fa-lg'></i></a>";
                // $htmlcontrol .= "<button type='button' class='btn btn-icon command-delete waves-effect waves-circle'><i class='fa fa-trash-o fa-lg'></i></button>";
                $htmlcontrol .= "</td>";
                $htmlcontrol .= "</tr>";
  ...
  function listmessages_parents($studentid)
  {
    $returnvalue = "";
    $user_permission="";
    $sql_query = "";
    ...
    try
    {
      ...
      if($select = $mysqli->prepare("CALL ". $this->sp_read_notifications_parents ." (?)"))
      {
        if($select->bind_param("s",$studentid))
        {
          if($select->execute())
          {
            ...
            $select->bind_result($id, $from, $subject, $subject_type, $message, $date,$type, $date_formated,$message_status);
            $select->store_result();
            ...
            if($select->num_rows > 0)
            {
              ...
              while($select->fetch())
              {
                ...
                $htmlcontrol .= "<td ". ($message_status == 1 ? "class=\"\"": "class=\"info\"") .">";
                $htmlcontrol .= "<div class='lv-title'><b>". htmlentities(strtoupper(substr($subject_type,0,1))) . htmlentities(strtolower(substr($subject_type,1,strlen($subject_type)-1)));
                $htmlcontrol .= " - (". htmlentities(strtoupper(substr($subject,0,1))) . htmlentities(strtolower(substr($subject,1,strlen($subject)-1))) .")</b></div>";
                ...
                $htmlcontrol .= "<input type='hidden' id='message_status' name='message_status' value='". $message_status . "'>";
                $htmlcontrol .= "<input type='hidden' id='message_status' name='message_status' value='". $message_status . "'>";
                // $htmlcontrol .= "<button type='button' class='btn btn-icon command-delete waves-effect waves-circle'><span class='fa fa-envelope'></span></button>";
  ...
  function readmessage_parent($id_message, $student_id)
  {
    $result = 0;
    ...
    try
    {
      ...
      if($stmt=$mysqli->prepare("CALL ". $this->sp_read_message_parent ."(?,?)"))
      {
        if($stmt->bind_param("is", $id_message, $student_id))
        {
          if($stmt->execute())
          {
            $result = 1;
  ...
  function getcountunreadmessages_parent($studend_id)
  {
    $sql_query = "";
    $unread_message="";
    $result = 0;
    ...
    try
    {
      ...
      if($select = $mysqli->prepare("CALL ". $this->get_count_unread_messages_parent." (?)"))
      {

        if($select->bind_param("s", $studend_id ))
        {
          if($select->execute())
          {
            $this->error = false;
            ...
            if($select->num_rows > 0)
            {
              while($select->fetch())
              {
                $htmlcontrol .= "<span class='charge'>". $count ."</span>";
              }
            }
          }
          else
  ...
  function delete_message($id_message,$dummy)
  {
    $result = 0;
    ...
    try
    {
      ...
      else
      {
        ...
        $mysqli=new mysqli($DBCreds->DBAddress, $DBCreds->DBUser, $DBCreds->DBPass, $DBCreds->DBSchema, $DBCreds->DBPort);
        if($stmt=$mysqli->prepare("CALL sp_delete_message (?)"))
        {

          if($stmt->bind_param("i", $id_message))
          {
            if($stmt->execute())
            $result = 1;
            $stmt->close();
