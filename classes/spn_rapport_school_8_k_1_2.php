...
class spn_rapport_school_8_k_1_2
{
						...
						if ($select->num_rows > 0) {
							...
							while ($select->fetch()) {									/* initialize last student id if whilecounter == 0 */
								if ($_while_counter == 0) {
									...
									$hojaActiva->setCellValue($_default_excel_klas, $klas_in);
									$hojaActiva->setCellValue($_default_excel_schooljaar, $schooljaar);
							...
							if ($this->debug) {
								...
								$hojaActiva->setCellValue("B50", $_debug_write_counter);
							}
						} else {
							if ($this->debug) {
	...
	function _writerapportdata_houding($schoolid, $schooljaar, $klas_in, $rap_in)
	{
		...
		try {
			...
			if ($select = $mysqli->prepare($sql_query)) {
				if ($select->bind_param("issssi", $schoolid, $schooljaar, $schooljaar, $klas_in, $klas_in, $rap_in)) {
					if ($select->execute()) {
						$select->store_result();
						$this->error = false;
						if ($select->num_rows > 0) {
	...
	function _writeeapportdata_verzuim($schoolid, $schooljaar, $klas_in, $rap_in)
	{
					...
					if ($select->num_rows > 0) {
						...
						while ($select->fetch()) {
							...
							while ($cont_verzuim < 4) {
								...
								switch ($cont_verzuim) {
									case 1:
										$hojaActiva->setCellValue($_colverzuim_laat, $telaat);
										// print "caso 1 ".$_colverzuim_laat." - ";
										break;
									case 2;
										$hojaActiva->setCellValue($_colverzuim_verzm, $absentie);
										// print "caso 2 ".$_colverzuim_omperking." - ";
										break;
									case 3;
										$hojaActiva->setCellValue($_colverzuim_huiswerk, $huiswerk);
										break;
									case 4;
										break;
		...
		header('Content-Disposition: attachment;filename="ttr.xlsx"');
