...
class spn_test
{
  ...
  function get_test($id_test, $id_student, $dummy)
  {
    ...
    if ($dummy)
    {
      $htmlcontrol .= "<table id=\"dataRequest-test" ."\" class=\"table table-striped\" data-table=\"yes\">";
    ...
    else
    {
      ...
      try
      {
        ...
        if($select = $mysqli->prepare("CALL ". $this->sp_read_test ." (?, ?)"))
        {
          if($select->bind_param("ii", ($id_test == "" || $id_test == 0 || $id_test == "null" ? null : $id_test) , $id_student))
          {
            if($select->execute())
            {
              $this->error = false;
