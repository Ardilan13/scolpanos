...
class spn_rapport_by_student_s10_s11_v2
{
	...
	function createrapport($schoolid, $schooljaar_rapport, $id_student, $generate_all_rapporten)
	{
		...
		$spreadsheet = $reader->load("../templates/Witte_kaart_2018_s10_s11_v2.xlsx");
		...
		if ($schooljaar_rapport == "All") {
						...
						if ($select->execute()) {
							...
							if ($select->num_rows > 0) {
								...
								while ($select->fetch()) {
									...
									$hojaActiva->setCellValue("C3", $studentName);
			...
			// $rapport_query = "SELECT distinct concat(s.firstname, ' ', s.lastname), c.gemiddelde,
			// v.vak_naam, v.id, c.rapnummer, c.klas, c.schooljaar
			// FROM
			// le_cijfers_ps c
			// inner join
			// le_vakken_ps v on
			...
			// students s on
			// s.id = c.studentid
			// WHERE c.studentid = ?  and gemiddelde >= 0 order by c.schooljaar, c.rapnummer, v.id asc;";
			...
			// 				if ($select->num_rows > 0) {
			// 					$result = 1;
			// 					$select->bind_result($studentName, $gemiddelde, $volledigenaamvak, $id, $rapnummer, $klas, $schooljaar);
			...
			// 						}
			// 						if ($returnvalue != null) {
			// 							$hojaActiva->setCellValue($colum_rapport . $returnvalue, $gemiddelde);
			// 						}
			// 						$hojaActiva->setCellValue("C3", $studentName);
		...
		header('Content-Disposition: attachment;filename="wittekaart.xlsx"');
						...
						if ($select->execute()) {
							...
							if ($select->num_rows > 0) {
								...
								while ($select->fetch()) {
									...
									$objPHPExcel->setActiveSheetIndex(0)->setCellValue("C3", $studentName);
