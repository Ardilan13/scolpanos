...
class spn_rapport_by_student
{
	...
	function createrapport($schoolid,$schooljaar,$studentid, $generate_all_rapporten){
		...
		$objPHPExcel = PHPExcel_IOFactory::load("../templates/Witte_kaart_SPK_2018.xlsm");
				...
				if($select=$mysqli->prepare($verzuim_query)) {
					if($select->execute())
					{
						...
						if($select->num_rows > 0)
						{
							...
							while($select->fetch())
							{
								$objPHPExcel->setActiveSheetIndex(0)->setCellValue($colum_schooljaar."51",$huiswerk);
								$objPHPExcel->setActiveSheetIndex(0)->setCellValue($colum_schooljaar."52", $telaat );
								$objPHPExcel->setActiveSheetIndex(0)->setCellValue($colum_schooljaar."53", $absentie );
							}
						}
					}
				}
