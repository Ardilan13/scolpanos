...
class spn_houding_mobile
{
  ...
  function createhouding($studentnumber, $schooljaar, $rapnummer, $klas, $user, $h1, $h2, $h3, $h4, $h5, $h6, $h7, $h8, $h9, $h10, $h11, $h12, $h13, $h14, $h15)
  {
    ...
    try {
      ...
      if ($stmt = $mysqli->prepare("insert into " . $this->tablename_houding . "(created,studentid,schooljaar,rapnummer,klas,user,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11,h12,h13,h14,h15) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")) {
  ...
  function listhouding($schoolid, $klas_in, $rap_in, $sort_order)
  {
    ...
    $sql_query = "SELECT s.id, h.id, s.class, s.firstname, s.lastname, s.sex, h.h1, h.h2, h.h3, h.h4, h.h5, h.h6, h.h7, h.h8, h.h9, h.h10, h.h11, h.h12, h.h13, h.h14, h.h15,
                  ...
                  FROM students s LEFT JOIN le_houding h ON s.id = h.studentid WHERE s.class = ? AND h.rapnummer = ? AND s.schoolid = ?
                  ORDER BY ";
    ...
    // $sql_query = "select s.id, h.id, s.class, s.firstname, s.lastname, s.sex, h.h1, h.h2, h.h3, h.h4, h.h5, h.h6, h.h7
    //               from students s left join le_houding h on s.id = h.studentid where s.class = ? and h.rapnummer = ? and s.schoolid = ?
    //               order by s.firstname ". $s->_setting_sort;
    ...
    try {
      ...
      if ($select = $mysqli->prepare($sql_query)) {
        if ($select->bind_param("sii", $klas_in, $rap_in, $schoolid)) {
          if ($select->execute()) {
            // Audit by Caribe Developers
            $spn_audit = new spn_audit();
            $UserGUID = $_SESSION['UserGUID'];
            ...
            $result = 1;
            $select->store_result();
            $select->bind_result($studentid, $houdingid, $klas, $firstname, $lastname, $sex, $h1, $h2, $h3, $h4, $h5, $h6, $h7);


            if ($select->num_rows > 0) {
              $htmlcontrol .= "<table id=\"dataRequest-houding\" class=\"table table-bordered table-colored table-houding\" data-table=\"yes\">";
              $htmlcontrol .= "<thead>";
              $htmlcontrol .= "<tr class=\"text-align-center\"><th>ID</th><th class=\"btn-m-w\">Naam</th><th>Gedrag <br />Lezen</th><th>Concentratie <br />Lezen</th><th>Werkverzorging <br />Lezen</th><th>Motivatie <br />Rek</th><th>Godsdient <br />Rek</th><th>Huiswerk</th><th>Expr</th></tr>";
              $htmlcontrol .= "</thead>";
              $htmlcontrol .= "<tbody>";
              ...
              while ($select->fetch()) {
                ...
                for ($y = 1; $y <= $index_colum; $y++) {
                  ...
                  if (($s->_setting_rapnumber_1 && $rap_in == 1) || ($s->_setting_rapnumber_2 && $rap_in == 2) || ($s->_setting_rapnumber_3 && $rap_in == 3)) {
                    $htmlcontrol .= "<select id=\"lblName$xx\" id_houding_table=\"$houdingid\"data-student-id=\"$studentid\" data-houding=\"h$y\" data-klas=\"$klas_in\" data-rapport=\"$rap_in\" class=\"form-control editable-select\">";
                  }
  ...
  function savehouding($schoolid, $studentid_in, $houding_number_in, $houding_value_in, $klas_in, $rap_in)
  {
    ...
    if ($houding_count == 1) {
      ...
      /* TODO: Validate  */
      if ($houding_number_in == "h1" || $houding_number_in == "h2" || $houding_number_in == "h3" ||  $houding_number_in == "h4" || $houding_number_in == "h5" || $houding_number_in == "h6" || $houding_number_in == "h7") {


        return $this->_updatehouding($schoolid, $studentid_in, $houding_number_in, $houding_value_in, $klas_in, $rap_in);
      }
    } else {
      ...
      if ($houding_number_in == "h1" || $houding_number_in == "h2" || $houding_number_in == "h3" ||  $houding_number_in == "h4" || $houding_number_in == "h5" || $houding_number_in == "h6" || $houding_number_in == "h7") {
        ...
        return $this->_inserthouding($schoolid, $studentid_in, $houding_number_in, $houding_value_in, $klas_in, $rap_in);
      }
    }
  }
  ...
  function _updatehouding($schoolid, $studentid_in, $houding_number_in, $houding_value_in, $klas_in, $rap_in)
  {
    ...
    try {
      ...
      if ($stmt = $mysqli->prepare("update" . chr(32) . $this->tablename_houding . chr(32) . "h join students s on h.studentid = s.id set" . chr(32) . "h." . $houding_number_in . chr(32) . " = ? where s.id = ? and s.class = ? and h.rapnummer = ?;")) {
        if ($stmt->bind_param("ssss", $houding_value_in, $studentid_in, $klas_in, $rap_in)) {
          if ($stmt->execute()) {
            // Audit by Caribe Developers
            $spn_audit = new spn_audit();
            $UserGUID = $_SESSION['UserGUID'];
  ...
  function _inserthouding($schoolid, $studentid_in, $houding_number_in, $houding_value_in, $klas_in, $rap_in)
  {
    ...
    try {
      ...
      if ($stmt = $mysqli->prepare("insert into" . chr(32) . $this->tablename_houding . chr(32) . "(studentid, created, schooljaar, rapnummer, klas, user," . chr(32) . $houding_number_in . ") values (?,?,?,?,?,?,?);")) {
        if ($stmt->bind_param("sssssss", $studentid_in, $_DateTime, $schooljaar_temp, $rap_in, $klas_in, $user_temp, $houding_value_in)) {
          if ($stmt->execute()) {
            // Audit by Caribe Developers
            $spn_audit = new spn_audit();
            $UserGUID = $_SESSION['UserGUID'];
  ...
  function listhoudingbystudent_hs($schooljaar, $studentid)
  {
    ...
    try {
      ...
      $sp_get_cijfers_by_student = "";
      $mysqli->set_charset('utf8');
      if ($stmt = $mysqli->prepare("CALL sp_get_houding_by_student_parent_hs (?,?)")) {
        if ($stmt->bind_param("ss", $schooljaar, $studentid)) {
          if ($stmt->execute()) {
            // Audit by Caribe Developers
            ...
            $result = 1;
            $stmt->bind_result($volledigenaamvak, $houdingid, $studentid, $schooljaar, $klas, $rapnummer, $h1, $h2, $h3, $h4, $h5, $h6, $h7, $h8, $h9, $h10, $h11, $h12, $h13, $h14, $h15, $h16, $h17, $h18, $h19, $h20, $h21, $h22, $h23, $h24, $h25, $studentduuid);
            $stmt->store_result();
