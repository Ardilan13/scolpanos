...
class spn_cijfers_hs3
{
            ...
            if($select->num_rows > 0)
            {
              ...
              while($select->fetch())
              {
                ...
                for($y=1;$y <= 20;$y++)
                {
                  ...
                  {
                    $cell='x'.$x.'y'.$y;
                    $htmlcontrol .= "<td><span id=\"lblName1\" disabled='true' data-row=\"$x\" id_cell_cijfer= \"$cell\" id_cijfer_table = \"$cijferid\" data-student-id=\"$studentid\" data-cijfer=\"c$y\" data-klas=\"$klas\" data-vak=\"$vak_in\" data-rapport=\"$rap_in\" class=\"editable\">$_cijfer_number</span></td>";
                  }
                  else
                  {
                    $cell='x'.$x.'y'.$y;
                    $htmlcontrol .= "<td><span id=\"lblName1\" data-row=\"$x\" id_cell_cijfer= \"$cell\" id_cijfer_table = \"$cijferid\" data-student-id=\"$studentid\" data-cijfer=\"c$y\" data-klas=\"$klas\" data-vak=\"$vak_in\" data-rapport=\"$rap_in\">$_cijfer_number</span></td>";
                  }
              ...
              if($stmt=$mysqli->prepare("CALL " . $this->sp_read_le_cijfersextra . " (?,?,?,?)"))
              {
                if($stmt->bind_param("ssis",$klas_in, $vak_in, $rap_in,$schooljaar))
                {
                  if($stmt->execute())
                  {
                    $stmt->bind_result($cijfersextraid,$klas,$schooljaar,$rapnummer,$vak,$oc1,$oc2,$oc3,$oc4,$oc5,$oc6,$oc7,$oc8,$oc9,$oc10,$oc11,$oc12,$oc13,$oc14,$oc15,$oc16,$oc17,$oc18,$oc19,$oc20,
  ...
  function getgemiddelde($schoolid,$studentid_in,$klas_in,$rap_in,$vak_in)
  {
    ...
    $sql_query = "select c.gemiddelde from le_cijfers c where c.studentid = $studentid_in and c.rapnummer = $rap_in and c.vak = $vak_in";
  ...
  function _updatecijfer($schooljaar, $schoolid,$id_cijfers,$studentid_in,$cijfer_number_in,$cijfer_value_in,$klas_in,$rap_in,$vak_in)
  {
    ...
    try
    {
      ...
      if ($cijfer_value_in == ''){
        $sql_query="update le_cijfers c join students_groups s on c.studentid = s.studentid set c." . $cijfer_number_in . " = NULL where s.studentid = $studentid_in and c.klas = '$klas_in' and s.vakid = $vak_in  and c.rapnummer = $rap_in and c.vak = $vak_in and c.ID = $id_cijfers;";
      }
      else{
        $sql_query="update le_cijfers c join students_groups s on c.studentid = s.studentid set c." . $cijfer_number_in . " = $cijfer_value_in where s.studentid = $studentid_in and c.klas = '$klas_in' and s.vakid = $vak_in  and c.rapnummer = $rap_in and c.vak = $vak_in and c.ID = $id_cijfers;";
  ...
  function _savecijfersgemiddelde($schooljaar,$schoolid,$studentid_in,$klas_in,$rap_in,$vak_in)
  {
    ...
    where c.klas = '$klas_in' and v.id = $vak_in
    and c.rapnummer = $rap_in
    and c.studentid = $studentid_in
    and c.schooljaar = '$schooljaar' order by s.firstname asc LIMIT 1";
    /* TODO: QUERY NEEDS TO BE FIXED , EL */
    ...
    try
    {
      ...
      if($select=$mysqli->prepare($sql_query)) {
        if($select->execute())
        {
          ...
          $select->bind_result($studentid,$cijferid,$klas,$firstname,$lastname,$sex,$c1,$c2,$c3,$c4,$c5,$c6,$c7,$c8,$c9,$c10,$c11,$c12,$c13,$c14,$c15,$c16,$c17,$c18,$c19,$c20,
          $w1,$w2,$w3,$w4,$w5,$w6,$w7,$w8,$w9,$w10,$w11,$w12,$w13,$w14,$w15,$w16,$w17,$w18,$w19,$w20,$gemiddelde);
          $select->store_result();
          ...
          if ($select->num_rows == 1) {
            ...
            while($select->fetch())
            {
              $gemid= $this->_gemiddeldebasis ($c1,$c2,$c3,$c4,$c5,$c6,$c7,$c8,$c9,$c10,$c11,$c12,$c13,$c14,$c15,$c16,$c17,$c18,$c19,$c20,
              ...
              //
              // print('Gemmindele Antes: <br>');
              // print $gemid.'<br>';

              if ($_SESSION['SchoolType']>=2){
                $gemid = round($gemid,1,PHP_ROUND_HALF_UP);
              }
              // print('Gemmindele DEspues: <br>');
              // print $gemid.'<br>';

              //$mysqli=new mysqli($DBCreds->DBAddress, $DBCreds->DBUser, $DBCreds->DBPass, $DBCreds->DBSchema, $DBCreds->DBPort);
              $sql_querygemid = "update  le_cijfers set gemiddelde =  $gemid  where id = $cijferid";
  ...
  function _savecijfersgemiddelde_by_group($schooljaar,$schoolid,$studentid_in,$klas_in,$rap_in,$vak_in)
  {
    $returnvalue = 0;
    $user_permission="";
    $sql_query = "";
    ...
    and cw.schooljaar = c.schooljaar
    and cw.rapnummer = c.rapnummer
    where c.groupname = '$klas_in' and v.id = $vak_in
    and c.rapnummer = $rap_in
    and c.studentid = $studentid_in
    and c.schooljaar = '$schooljaar' order by s.firstname asc LIMIT 1";
    /* TODO: QUERY NEEDS TO BE FIXED , EL */
  ...
  function _gemiddeldebasis ($c1,$c2,$c3,$c4,$c5,$c6,$c7,$c8,$c9,$c10,$c11,$c12,$c13,$c14,$c15,$c16,$c17,$c18,$c19,$c20,
  $w1,$w2,$w3,$w4,$w5,$w6,$w7,$w8,$w9,$w10,$w11,$w12,$w13,$w14,$w15,$w16,$w17,$w18,$w19,$w20)
  {
    ...
    if ($index > 0 && $samen > 0)
    {
      ...
      $Xgemid = ($samen/$index);
      // echo "<br>this is the final Gemiddelde<br> ";
      // echo $Xgemid;
      // echo "<br>";
  ...
  //Caribe Dev
  function create_le_cijfer_student($schooljaar,$rapnummer,$group_name,$schoolID, $vakid){
    $result = 0;
    require_once("DBCreds.php");
    $DBCreds = new DBCreds();
    date_default_timezone_set("America/Aruba");
    ...
    try
    {
      ...
      for ($i = 0; $i < $length; $i++) {
        ...
        $mysqli=new mysqli($DBCreds->DBAddress, $DBCreds->DBUser, $DBCreds->DBPass, $DBCreds->DBSchema, $DBCreds->DBPort);
        if($stmt=$mysqli->prepare("CALL sp_create_le_cijfers_student_hs3 (?,?,?,?,?,?)"))
        {
          if($stmt->bind_param("sssssi",$group_name, $schooljaar,$rapnummer,$klasArray[$i],$schoolID, $vakid))
          {
            if($stmt->execute())
            {
              /*
  ...
  function get_cijfers_graph($schooljaar, $schoolid, $dummy){
    ...
    if ($dummy){
      ...
      try{
        ...
        if ($select =$mysqli->prepare("CALL sp_read_le_cijfer_graph_hs (?,?)")){
            ...
            if ($dummy){
                    ...
                    if($stmt=$mysqli->prepare("CALL " . $this->$sp_get_vak_by_teacher . "(?,?,?)"))
                    {
                      if ($stmt->bind_param("iss",$schoolid,$userGUID, $klas))
                      {
                        if($stmt->execute())
                        {
                          $this->error = false;
                          $result=1;
                          $stmt->bind_result($id_vak, $name_vak);
                          $stmt->store_result();
                          ...
                          if($stmt->num_rows > 0)
                          {
                            ...
                            while($stmt->fetch())
                            {
                              $htmlcontrol .= "<option value=". htmlentities($id_vak) .">". htmlentities($name_vak) ."</option>";
                            }
                            $htmlcontrol .= "</select>";
                          }
                          else
              ...
              function createcijferswaarde_first_time($klas,$schoolid, $schooljaar, $rapnummer, $vak,$c1,$c2,$c3,$c4,$c5,$c6,$c7,$c8,$c9,$c10,$c11,$c12,$c13,$c14,$c15,$c16,$c17,$c18,$c19,$c20)
              {
                ...
                try
                {
                  ...
                  if($stmt=$mysqli->prepare("CALL sp_create_le_cijferswaarde_first_time_hs3 (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"))
                  {
              ...
              function listcijfers_hs($schoolid,$group,$vak_in,$rap_in,$sort_order,$schooljaar)
              {
                $returnvalue = "";
                $user_permission="";
                $sql_query = "";
                ...
                s.id, c.id, s.class, s.lastname,s.firstname, s.sex,
                c.c1, c.c2, c.c3, c.c4, c.c5, c.c6, c.c7, c.c8, c.c9, c.c10,
                c.c11, c.c12, c.c13, c.c14, c.c15, c.c16, c.c17, c.c18, c.c19, c.c20, c.gemiddelde
                FROM students s
                LEFT JOIN le_cijfers c ON s.id = c.studentid
                inner join students_groups sg on sg.studentid = s.id
                LEFT JOIN le_vakken v ON c.vak = v.id WHERE
                sg.vakid =  $vak_in and c.groupname = '$group' AND v.id = $vak_in AND c.rapnummer = $rap_in AND c.schooljaar = '$schooljaar' Order By";
                  ...
                  if($select=$mysqli->prepare($sql_query)) {
                    if($select->execute())
                    {
                      ...
                      if($select->num_rows > 0)
                      {
                        ...
                        if($stmtcijferswaarde=$mysqli->prepare("CALL " . $this->sp_read_le_cijferswaarde . " (?,?,?,?)"))
                        {
                          if($stmtcijferswaarde->bind_param("ssis",$group, $vak_in, $rap_in, $schooljaar))
                          {
                            if($stmtcijferswaarde->execute())
                            {
                              $stmtcijferswaarde->bind_result($cijferswaardeid,$klas,$schooljaar,$rapnummer,$vak,$cijferswaarde1,$cijferswaarde2,$cijferswaarde3,$cijferswaarde4,$cijferswaarde5,$cijferswaarde6,$cijferswaarde7,$cijferswaarde8,$cijferswaarde9,$cijferswaarde10,$cijferswaarde11,$cijferswaarde12,$cijferswaarde13,$cijferswaarde14,$cijferswaarde15,$cijferswaarde16,$cijferswaarde17,$cijferswaarde18,$cijferswaarde19,$cijferswaarde20);
