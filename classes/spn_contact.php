...
class spn_contact{
  ...
  $id_contact)
  {
    ...
    try
    {
      ...
      {
        if($stmt->bind_param("iisissssssisss",$id_contact,$tutor,$type,$id_number_contact,$full_name,$address,$email,$mobile_phone,$home_phone,$work_phone,$work_phone_ext,$company,$position_company,$observations))
        {
          if($stmt->execute())
          {   //Audit by Caribe Developers
            require_once ("spn_audit.php");
  ...
  function createcontact_parent($tutor,$type,$id_number,$full_name,$address,$email,$mobile_phone,$home_phone,$work_phone,$work_phone_ext,$company,$position_company,$observations,$id_family,$dummy)
  {
    ...
    if($dummy)
    {
      ...
      try
      {
        ...
        {
          if($stmt->bind_param("isissssssissss",$tutor, $type, $id_number, $full_name, $address, $email, $mobile_phone, $home_phone, $work_phone, $work_phone_ext, $company, $position_company, $observations, $id_family))
          {
            if($stmt->execute())
            {
              /*
