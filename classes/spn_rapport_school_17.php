...
class spn_rapport_school_17
{
	...
	public $sp_read_opmerking_by_rapport = "sp_read_opmerking_by_rapport";
		...
		$this->rapport_modified = $this->rapport_reader->load("../templates/17_monplaisir_rapport_file.xlsm");
		...
		if($exporttobrowser)
		{
			...
			header('Content-Disposition: attachment; filename="MonPlaisir_rapport_file.xlsm"');
			/* add headers for output to browser */
		}
						...
						$select->bind_result(
							...
							while($select->fetch())
							{
								...
								if($rap_in == 1){
									...
									if($vakid_out==552435 || $vakid_out==552449 ||$vakid_out==552463 ||$vakid_out==552477){
										$this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue("EW" . (string)$_current_student_start_row,$gemiddelde_out ); }
									
								} 
								if($rap_in == 2){
									$colgemiddelde = PHPExcel_Cell::stringFromColumnIndex(($x_index_out+1)). (string)$_current_student_start_row;
									if($vakid_out==552435 || $vakid_out==552449 ||$vakid_out==552463 ||$vakid_out==552477){
										$this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue("EY" . (string)$_current_student_start_row,$gemiddelde_out ); }
									if($_while_counter > 0){
								...
								if($rap_in == 3){
									...
									if($vakid_out==552435 || $vakid_out==552449 ||$vakid_out==552463 ||$vakid_out==552477){
										$this->rapport_modified->setActiveSheetIndex($this->activesheet_index)->setCellValue("FA" . (string)$_current_student_start_row,$gemiddelde_out ); }
									if($_while_counter > 0){
