...
class spn_rapport_school_12
{
	...
	function _exportexcelfile($exporttobrowser = true)
	{
		...
		if($exporttobrowser)
		{
			...
			header('Content-Disposition: attachment; filename="ceque_rapport_file.xls"');
			/* add headers for output to browser */
		}
	...
	function _writerapportdata_cijfers($schoolid,$schooljaar,$klas_in,$rap_in)
	{
						...
						$select->bind_result(
							...
							while($select->fetch())
							{
								if($_while_counter == 0)
								{
									...
									$tutor = $this->_writetutorName($klas_in,$schoolid);
		...
		function _writetutorName($klas_in, $schoolId)
		{
