...
<head>
	<title>Aplicacion Form Amalia</title>
...
<main>
    <div class="container">
		<div class="form-bg"'>
            ...
            <h1>Prinses Amalia School</h1>
            <h2>Aplicacion Scol 2022-2023:</h2>
            <?php $DBCreds = new DBCreds();
$mysqli = new mysqli($DBCreds->DBAddress, $DBCreds->DBUser, $DBCreds->DBPass, $DBCreds->DBSchema, $DBCreds->DBPort);
$mysqli->set_charset('utf8');
$query = "SELECT ID,status,fam,nomber,sexo,f_nacemento,districto,ruman,collega,number_mayor1,number_mayor2,email_mayor1,email_mayor2,prome_preferencia,segundo_preferencia FROM aplicacion_forms WHERE (prome_preferencia = 3 OR segundo_preferencia = 3) ORDER BY status ASC, fam ASC, nomber ASC";
            ...
            <table class="table table-striped table-bordered">
                ...
                <?php while ($row = mysqli_fetch_assoc($resultado)) {?>
                    <tr>
                        ...
                        <th class="sex"><?php if ($row["sexo"] == 1) {echo 'M';} else {echo 'F';}?></th>
                        ...
                        <th class="sex"><?php if ($row["ruman"] == 1) {echo 'Si';} else {echo 'No';}?></th>  
                        <th class="sex"><?php if ($row["collega"] == 1) {echo 'Si';} else {echo 'No';}?></th> -->  
                        <th class="datum"><?php echo $row["number_mayor1"] ?></th>
                        <th class="datum"><?php echo $row["number_mayor2"] ?></th>
                        <th style="font-size:12px;"><?php echo $row["email_mayor1"] ?></th>
                        <th style="font-size:12px;"><?php echo $row["email_mayor2"] ?></th>
                        <th><?php if($row["prome_preferencia"]==3){ echo '1';} else{ echo '2';} ?></th>
                        <form action="aplicacion_form-update.php" method="POST">
                        <input name="id" hidden type="text" value="<?php echo $row["ID"]; ?>">
                        <input name="scol" type="text" hidden value="3">
