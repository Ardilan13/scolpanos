...
class spn_user_hs_account
{
  ...
  function delete_klas_and_vack_hs_account($user_hs_email,$dummy)
  {
    $result = 0;
    ...
    try
    {
      ...
      else
      {
        $mysqli=new mysqli($DBCreds->DBAddress, $DBCreds->DBUser, $DBCreds->DBPass, $DBCreds->DBSchema, $DBCreds->DBPort);
          // print('diferente');
          if($stmt=$mysqli->prepare("DELETE FROM app_useraccounts WHERE Email  = ?;"))
          {
            if($stmt->bind_param("s", $user_hs_email))
            {
              if($stmt->execute())
              {
                $result = 1;
