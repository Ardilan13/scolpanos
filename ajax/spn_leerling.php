...
class spn_leerling
{
  ...
  function edit_leerling($studentid, $_DateTime, $schoolid, $idnumber, $studentnumber, $class, $enrollmentdate, $firstname, $lastname, $sex, $_dob, $birthplace, $address, $azvnumber, $azvexpiredate, $phone1, $phone2, $colorcode, $status, $roepnaam, $nationaliteit, $vezorger, $email, $ne, $pa, $en, $sp,$vo, $spraak, $gehoor, $gezicht, $motoriek, $huisarts, $huisartsnr, $anders, $voorletter, $phone3, $vorigeschool, $bijzondermedischeindicatie, $notas, $id_family,$datum_uitschijving)
  {
    ...
    try
    {
      $mysqli=new mysqli($DBCreds->DBAddress, $DBCreds->DBUser, $DBCreds->DBPass, $DBCreds->DBSchema, $DBCreds->DBPort);
      $mysqli->set_charset('utf8');
      $sql_query_update = "UPDATE students SET idnumber = 123456789 WHERE id = 1607";
      echo mysqli_query($mysqli, $sql_query_update);

      /* if($stmt=$mysqli->prepare($sql_query_update))
      {
        //if($stmt->bind_param("si",$_DateTime, $studentid))
        if($stmt->bind_param("sisssssssssssssssissssiiiisiiiissssssssiis",
        $_DateTime,$schoolid,$idnumber,$studentnumber,$class,$enrollmentdate,$firstname,$lastname,$sex,$_dob,$birthplace,
        $address, $azvnumber, $azvexpiredate,$phone1,$phone2,$colorcode,$status,$roepnaam,$nationaliteit,$vezorger,$email,
        $ne,$en,$sp,$pa,$vo,$spraak,$gehoor,$gezicht,$motoriek,$huisarts,$huisartsnr,$anders,$voorletter,$phone3,$vorigeschool,
        $bijzondermedischeindicatie,$notas,$id_family,$studentid, $datum_uitschijving))
        {
          if($stmt->execute())
          {
  ...
  function get_all_students_array_by_klas($klas)
  {
    ...
    $studentlist = array();
    mysqli_report(MYSQLI_REPORT_STRICT);
    $sql_query = "SELECT id,studentnumber,firstname,lastname,sex,dob,class  FROM students where class = ?  and schoolid = ? and status = 1";
