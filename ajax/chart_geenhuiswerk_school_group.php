	...
	if(isset($_SESSION["UserRights"]) && isset($_SESSION["SchoolID"]))
	{
		if($_SESSION["UserRights"] == "DOCENT" || $_SESSION["UserRights"] == "ASSISTENT")
		{
			if(isset($_SESSION["Class"]))
			{
				...
				$count_geenhuiswerk = $s->dash_geenhuiswerk_school_group($u->converttomysqldate($date_school_group), $schoolid,$_SESSION["Class"]);
				//print $s->mysqlierror;
		...
		else if($_SESSION["UserRights"] == "BEHEER" || $_SESSION["UserRights"] == "ADMINISTRATIE" || $_SESSION["UserRights"] == "ONDERSTEUNING")
		{
			$s = new spn_school();
			// function dash_geenhuiswerk($schoolid_in,$klas_in)
			$count_geenhuiswerk = $s->dash_geenhuiswerk_school_group($u->converttomysqldate($date_school_group), $schoolid,"ALL");
			//print $s->mysqlierror;
