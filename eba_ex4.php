            ...
            <div class="container container-fs">
                <div class="row">
                    <div class="default-secondary-bg-color col-md-12 full-inset brd-bottom clearfix">
                        <h1 class="primary-color">EX. 4-M</h1>
                        <div style="display: flex; gap: 30rem;">
                            <button id="btn_eba_export" class="btn btn-primary btn-m-w btn-s-h">Export</button>
                            <p id="aviso" style="background-color: #ffdc66; border: 1px solid black; max-width: 400px; margin: 0; line-height: normal;">Lijst van kandidaten voor de HERKANSING 2de tijdvak.Tevens lijst van kandidaten, die om een geldige reden verhinderd waren het examen te voltooien. Inzenden binnen 1 week na elke uitslag.</p>
                        </div>
                                ...
                                <div class="box box_form">
                                    <div class="box-content full-inset">
                                        <div id="table" class="data-display table-responsive">
                                            ...
                                            $get_personalia1 = "SELECT e.id,e.*,s.id as studentid FROM eba_ex e INNER JOIN personalia p ON e.id_personalia = p.id INNER JOIN students s ON p.studentid = s.id WHERE e.schoolid = $schoolid AND e.schooljaar = '$schooljaar' AND s.SchoolID = $schoolid AND e.type = 4;";
                                            ...
                                            <?php } else {
                                                ...
                                                while ($row = mysqli_fetch_assoc($result)) {
                                                  ...
                                                  ";
                                                    ...
                                                    while ($row1 = mysqli_fetch_assoc($result1)) {
                                                        $create_ex1 = "INSERT INTO eba_ex (type,schoolid,schooljaar,id_personalia,e1,e2,e3,e4,e5,e6,e7,e8,e9,e10,e11,e12) VALUES (4,$schoolid,'$schooljaar',$personalia,'" . $row1["ne"] . "','" . $row1["en"] . "','" . $row1["sp"] . "','" . $row1["pa"] . "','" . $row1["wi"] . "','" . $row1["na"] . "','" . $row1["sk"] . "','" . $row1["bi"] . "','" . $row1["ec"] . "','" . $row1["ak"] . "','" . $row1["gs"] . "','" . $row1["re"] . "')";
                                                        mysqli_query($mysqli, $create_ex1);
                                                    }
                                                }
                                                header("Refresh:0");
