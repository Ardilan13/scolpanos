						...
						<div class="default-secondary-bg-color col-md-12 inset filter-bar brd-bottom clearfix">
							<form id="form-vrapportexport" class="form-inline form-data-retriever" name="filter-vak" role="form" method="GET" action="/dev_tests/test_exportexcel.php">
								<fieldset>
									...
									<div class="form-group">
										<label for="cijfers_rapnr_lijst">Rapport</label>
										<select class="form-control" name="Rapnr_rapport" id="Rapnr_rapport">
											<!-- Options populated by AJAX get -->
...
<script>
	$("#btn_tussen_rapport").click(function(e) {
		e.preventDefault();
		window.open("print_se_kaart.php?schoolJaar=" + $('#schooljaar_rapport').val() + "&klas=" + $('#rapport_klassen_lijst').val() + "&rap=" + $('#Rapnr_rapport').val() + "&studentid=" + $('#opmerking_student_name').val());
	});
